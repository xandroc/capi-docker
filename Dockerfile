...
RUN wget https://github.com/postmodern/ruby-install/releases/download/v0.9.0/ruby-install-0.9.0.tar.gz && tar -xzvf ruby-install-0.9.0.tar.gz && cd ruby-install-0.9.0 && make install

RUN ruby-install ruby 2.7.8
...
RUN DEBIAN_FRONTEND=noninteractive apt-get install zip lsof tzdata -y

# gem install pg -v '1.2.3' --source 'https://rubygems.org/'

# maybe gem install bundler for postgres client
RUN apt-get install postgresql-client -y
ADD run-tests /run-test
